...
std::pair<bool, Board::Color> Board::isEmpty(Index specifiedLocation) {
...
void Board::swapPawnWith(const Index& pawnIndex, wchar_t* character, const Index& newLocation) {
    const auto[y, x] = pawnIndex;
    const auto[Y, X] = newLocation;
    Color PieceColor = m_pieceList[y][x]->getColor();
    PieceTypes pieceType = determineCharacter(character);

    if (pieceType == PieceTypes::Rook) {
        const wchar_t* myChar = character;
        std::string color = (PieceColor == Color::Black) ? "Black" : "White";
...
PieceTypes Board::determineCharacter(const wchar_t* character){
    ...
    if (*character == *BlackRook || *character == *WhiteRook) {
        return PieceTypes::Rook;
    } 

     if (*character == *BlackKnight || *character == *WhiteKnight) {
        return PieceTypes::Knight;
    }
    
    if (*character == *BlackQueen || *character == *WhiteQueen) {
